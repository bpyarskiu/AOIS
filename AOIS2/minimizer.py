...
class Minimizer:
    ...
    def _remove_redundant_implicants(
        self, implicants: List[str], minterms: List[int]
    ) -> List[str]:
        """
    ...
    def minimize_quine_dnf(self, verbose: bool = True) -> str:
        ...
        essential = self._remove_redundant_implicants(
            list(all_implicants), self.minterms
        )
    ...
    def minimize_quine_knf(self, verbose: bool = True) -> str:
        ...
        # Удаление лишних
        essential = self._remove_redundant_implicants(
            list(all_implicants), self.maxterms
        )

        # Формируем КНФ (с инверсией)
        result_terms = [
            self._implicant_to_expression(imp, negative=True) for imp in essential
        ]
        result = " & ".join(result_terms)
